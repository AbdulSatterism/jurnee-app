  ...
  const post = await Post.aggregate([
    ...
    // if userId exist in attenders array so add another filed isAttender: true else false
    {
      $addFields: {
        isAttender: {
          $cond: [
            { $in: [new mongoose.Types.ObjectId(userId), '$attenders._id'] },
            true,
            false,
          ],
        },
...
const globalAllPost = async (query: IQuery) => {
  const {
    page = '1',
    limit = '10',
    category,
  ...
  //  Distance filter (if query location provided)
  const queryLat = lat ? parseFloat(lat) : null;
  const queryLng = lng ? parseFloat(lng) : null;
  const maxDist = maxDistance ? parseFloat(maxDistance) : 80470; // default 80.47km
  const ratingNum = typeof rating === 'string' ? parseFloat(rating) : rating;
...
const globalPostDetails = async (postId: string) => {
  const post = await Post.aggregate([
    { $match: { _id: new mongoose.Types.ObjectId(postId) } },
  ...
  await Post.updateOne({ _id: postId }, { $set: { views: updatedPost.views } });
...
// my boosted post by author

const myBoostedPost = async (
  userId: string,
  query: Record<string, unknown>,
) => {
  const { page, limit } = query;
  ...
  const [post, total] = await Promise.all([
    Post.find({ author: userId, boost: true })
      .populate('author', 'name image _id')
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(size)
      .lean(),
    Post.countDocuments({ author: userId, boost: true }),
  ]);
